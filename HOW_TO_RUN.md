...
## SSH to Jetson
```bash
sshpass -p '123' ssh unitree@172.20.10.6
```
...
## One-liner from PC
```bash
sshpass -p '123' ssh -o ServerAliveInterval=10 unitree@172.20.10.6 "cd ~/come-here && source /opt/ros/humble/setup.bash && source ~/go2_ws/install/setup.bash && export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp && export CYCLONEDDS_URI=file:///home/unitree/unitree_ros2/cyclonedds_ws/src/cyclonedds.xml && PYTHONPATH=~/come-here/come_here_audio:\$PYTHONPATH python3 -u come_here_audio/scripts/hear_and_rotate_demo.py"
```
