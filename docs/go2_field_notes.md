...
## 9. What this changes for come-here

come-here is audio-visual: it needs a microphone array and a camera, and **neither was
present on the payload's USB bus in the most recent session** (section 8). The bring-up
must therefore:

- Fail the preflight loudly on a missing mic or camera rather than starting a node that
  silently produces no audio, which looks identical to a quiet room.
- Never assume `/frontvideostream` substitutes for a depth camera. It is a compressed
  front stream at 28.6 Hz, not a calibrated RGB-D source.

The command path is the Sport API (section 4), so the `mcf`-only rule applies and the
gait envelope bounds what a "come here" motion can actually request: below `vx = 0.5` the
stock policy does not produce a clean trot, so a slow approach will look worse on video
than a normal-speed one.
